...
class Userprofile extends Controller
{
    ...
    /**
     * Export profil pengguna sebagai file PDF.
     * URL: GET /export/{id}/pdf
     */
    public function exportPdf($id)
    {
        if (empty($id)) {
            return redirect()->back()->withErrors('ID pengguna tidak valid.');
        }

        // Ambil semua data profil berdasarkan ID
        $personal_information   = PersonalInformation::find($id);
        if (empty($personal_information)) {
            abort(404, 'Profil pengguna tidak ditemukan.');
        }

        $info['personal_info']   = $personal_information->toArray();
        $info['contact_info']    = ContactInformation::where('user_id', $id)->first()?->toArray() ?? [];
        $info['education_info']  = Education::where('user_id', $id)->get()->toArray();
        $info['experience_info'] = Experience::where('user_id', $id)->get()->toArray();
        $info['project_info']    = Projects::where('user_id', $id)->get()->toArray();
        $info['skill_info']      = Skills::where('user_id', $id)->get()->toArray();
        $info['language_info']   = Languages::where('user_id', $id)->get()->toArray();
        $info['interest_info']   = Interests::where('user_id', $id)->get()->toArray();

        // Render Blade ke PDF
        $pdf = Pdf::loadView('cetak_pdf', ['info' => $info])
                  ->setPaper('a4', 'portrait')
                  ->setOptions([
                      'isHtml5ParserEnabled' => true,
                      'isRemoteEnabled'      => false, // gambar via path lokal, bukan URL
                      'defaultFont'          => 'DejaVu Sans',
                      'dpi'                  => 150,
                  ]);

        $namaFile = 'CV_' . str_replace(' ', '_', $info['personal_info']['first_name'])
                  . '_' . str_replace(' ', '_', $info['personal_info']['last_name'])
                  . '_' . date('Ymd') . '.pdf';

        // Otomatis terunduh di browser
        return $pdf->download($namaFile);
    }

    /**
     * Export profil pengguna sebagai CV ATS-Friendly PDF.
     * URL: GET /export/{id}/ats-pdf
     *
     * Perbedaan dari exportPdf:
     *  - Tidak menyertakan foto (ATS tidak bisa baca gambar)
     *  - Template cv_ats.blade.php: font standar, bullet list, tanpa grafik
     *  - Menggunakan Helvetica (font ATS-safe yang didukung dompdf built-in)
     */
    public function exportAtsPdf($id)
    {
        if (empty($id)) {
            return redirect()->back()->withErrors('ID pengguna tidak valid.');
        }
        ...
        // Render template ATS ke PDF
        $pdf = Pdf::loadView('cv_ats', ['info' => $info])
                  ->setPaper('a4', 'portrait')
                  ->setOptions([
                      'isHtml5ParserEnabled' => true,
                      'isRemoteEnabled'      => false,
                      // Helvetica: built-in dompdf, ATS-safe, selectable text
                      'defaultFont'          => 'Helvetica',
                      'dpi'                  => 150,
                      // Pastikan teks bisa di-select (bukan gambar)
                      'isFontSubsettingEnabled' => true,
                  ]);

        $namaFile = 'CV_ATS_'
                  . str_replace(' ', '_', $info['personal_info']['first_name'])
                  . '_' . str_replace(' ', '_', $info['personal_info']['last_name'])
                  . '_' . date('Ymd') . '.pdf';
